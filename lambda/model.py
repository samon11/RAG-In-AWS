...
def insert_document(bucket, key, chunk_count):
    blob_url = f"s3://{bucket}/{key}"
    title = os.path.basename(key)

    # insert document if it doesn't exist
    existing = Document.select().where(Document.blob_url == blob_url)
    if existing.exists():
        return existing.get().document_id
    
    document = Document(blob_url=blob_url, chunk_count=chunk_count, title=title)
    document.save()
